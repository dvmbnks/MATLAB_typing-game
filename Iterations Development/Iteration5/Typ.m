...
% Last Modified by GUIDE v2.5 17-Apr-2023 21:37:41
...
% Using a For loop to account for each second. 
for i = 1:60
    % Displaying to ensure time is running. 
    disp(".")
    % To ensure the code runs after every second. 
    pause(1)
    % Reducing timer value. 
    valTimers = valTimers - 1;
    % Setting the Timer string.
    set(handles.timer, 'string', valTimers);
    % When timer hits 60.
    if i == 60
        % Turn off input. 

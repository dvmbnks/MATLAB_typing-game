...
% Last Modified by GUIDE v2.5 24-Apr-2023 12:57:39
...
roundGame = str2double(get(handles.text22,'string'));
...
set(handles.text22, 'string',roundGame);
...
for i = 1:60
 % For each second. 
    ...
    if i == 60
        ...
        if getSwitch == 1
            ...
            % After inital information exists. 
                ...
                wpm = importedData(1,1:roundGame-1);
                % Collecting second line of the Matrix.                 
                accWord = importedData(2,1:roundGame-1);
                % Creating a new vector element with collected information. 
                wpm(roundGame) = scoring;
            end
    ...
    writematrix([wpm;accWord], 'MyFile.txt')

    % Information for the Message Box.
    A = sprintf("You made %.f charachter mistakes.", mistakes);
    C = sprintf("You got %.f charachters correct!", letters);
    D = sprintf("You have %.f percent accuracy!", accWord(roundGame));
    E = sprintf("You got %.f Words Per Min!", wpm(roundGame));
    % Pausing for the user to understand the game has ended. 
    pause(1)
     % Creating an alert box about the new information.
    msgbox(["Game Over";A;C;D;E]);

    % End Message.
    varEnd = "Game Ended";
    set(handles.text12, 'string',varEnd);
    set(handles.text12,'Foreground','RED')
    end
end
...
wpm = importedData(1,1:roundGame);
% Collecting bottom line of the Matrix as Accuracy. 
accWord = importedData(2,1:roundGame);
